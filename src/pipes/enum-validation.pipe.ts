...
export class EnumValidationPipe<T> implements PipeTransform<T> {
  constructor(private readonly enumType: Record<string, unknown>, private readonly options?: EnumValidationPipeOptions<T>) {}
  ...
  transform(value: unknown, metadata: ArgumentMetadata): T {
    ...
    if (!Object.values(this.enumType).includes(value)) {
      const errorMessage = `the value ${value} from field ${metadata.data} is not valid. Acceptable values: ${Object.values(
        this.enumType,
      )}`;
